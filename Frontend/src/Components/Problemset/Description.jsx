...
function Description({ problem }) {
    // Debug log to see the structure of the problem prop
    console.log('Problem data in Description component:', problem);

    if (!problem) {
        return <div className="text-yellow-500 p-4">No problem data available.</div>;
    }
    ...
    const renderSection = (title, content, isCode = false, isConstraints = false) => {
        if ((!content || (Array.isArray(content) && content.length === 0)) && !isConstraints) {
            return null;
        }
