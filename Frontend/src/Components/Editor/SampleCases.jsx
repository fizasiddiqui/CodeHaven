...
function SampleCases({ example_cases = [] }) {
    ...
    // If there are no example cases, don't render anything
    if (!example_cases || example_cases.length === 0) {
        return (
            <div className="bg-gray-700 p-4 rounded-lg mb-4">
                <h3 className="text-xl font-semibold text-white mb-2">Example Cases</h3>
                <p className="text-gray-300">No example cases available.</p>
            </div>
        );
    }
    ...
    return (
        <div>
            ...
            <div className="mb-4">
                {example_cases.map((_, index) => (
                    <button 
                        key={index} 
                        className={`px-3 py-1 rounded-lg mr-2 ${visibleIndex === index ? 'bg-blue-600 text-white' : 'bg-gray-500 text-gray-300'}`}
...
SampleCases.propTypes = {
    example_cases: PropTypes.arrayOf(
        PropTypes.shape({
