...
function EditorBox({ problem }) {
    ...
    const loadTemplateAndLanguage = useCallback(async () => {
        try {
            ...
            if (data?.template && data?.default_language) {
                const newTemplate = data.template;
                const newLanguage = data.default_language;
                setTemplate(newTemplate);
                setLanguage(newLanguage);
                setCode(newTemplate[newLanguage] || defaultCodes[newLanguage] || defaultCodes.cpp);
            } else {
                // Fallback to default values if API call fails or returns invalid data
                setTemplate(defaultCodes);
                setLanguage('cpp');
...
EditorBox.propTypes = {
  problem: PropTypes.shape({
    ...
    example_cases: PropTypes.arrayOf(PropTypes.object),
    // Add other expected problem properties here
  })
};
